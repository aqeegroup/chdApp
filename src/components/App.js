...
class App extends Component {
  ...
  _renderContent(title, component) {
    console.log('_renderContent', title);
  ...
  press(selected){
    const { actions } = this.props;
    actions.selecteTab(selected);
  }
  render() {

    // console.log(this.props);

    const { selectedTab, actions, loading } = this.props;
    // console.log(selectedTab, 'app');
    // console.log(state.bindList.selectedTab);
    return (
        <View style={{flex: 1}}>
          <TabBarIOS
            ...
            <TabBarIOS.Item
              ...
              selected={selectedTab === 'chengji'}
              onPress={this.press.bind(this, 'chengji')}>
              {selectedTab === 'chengji' && this._renderContent('成绩', Score)}
            </TabBarIOS.Item>

            {/*<TabBarIOS.Item
              title="课表"
              icon={{uri: kebiao64Icon, scale: 2.5}} 
              selected={selectedTab === 'kebiao'}
              onPress={this.press.bind(this, 'kebiao')}>
              {selectedTab === 'kebiao' && this._renderContent('课表', My)}
            </TabBarIOS.Item>*/}
            ...
            <TabBarIOS.Item
              ...
              {selectedTab === 'my' && this._renderContent('我', My)}
            </TabBarIOS.Item>
          </TabBarIOS>
          <BindModal actions={actions}/>
